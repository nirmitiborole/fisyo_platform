...
class FingersPage extends StatefulWidget {
  ...
  FingersPage({required this.device});
...
class _FingersPageState extends State<FingersPage> {
  BluetoothCharacteristic? _fingersCharacteristic;
      ...
      for (var service in services) {
        for (var characteristic in service.characteristics) {
          if (characteristic.uuid.toString() == 'beb5483e-36e1-4688-b7f5-ea07361b26a8') {
            setState(() {
              _fingersCharacteristic = characteristic;
            });
            print('✓ Found fingers characteristic: ${characteristic.uuid}');
            return;
          }
        }
      }
  ...
  Future<void> startReading() async {
    if (_fingersCharacteristic == null) {
    ...
    _readTimer = Timer.periodic(Duration(milliseconds: 500), (_) async {
      ...
      try {
        ...
        final data = await _fingersCharacteristic!.read().timeout(
          Duration(seconds: 2),
          onTimeout: () {
            print('Read timeout');
            return [];
        ...
        if (data.isNotEmpty && mounted) {
          ...
          print('Received Values: ${values.join(', ')}');
        }
      } catch (e) {
    ...
    print('Started reading data every 500ms.');
  }
  ...
  void _updateGraphBounds() {
    ...
    print('Updated Graph Bounds: MinY = $_minY, MaxY = $_maxY');
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Fingers: ${widget.device.name}'),
        actions: [
          Padding(
