...
class WristPage extends StatefulWidget {
  ...
  WristPage({required this.device});
...
class _WristPageState extends State<WristPage> {
  BluetoothCharacteristic? _wristCharacteristic;
      ...
      for (var service in services) {
        for (var characteristic in service.characteristics) {
          if (characteristic.uuid.toString() == 'beb5483e-36e1-4688-b7f5-ea07361b26a8') {
            setState(() {
              _wristCharacteristic = characteristic;
            });
            print('✓ Found wrist characteristic: ${characteristic.uuid}');
            return;
          }
        }
      }
  ...
  Future<void> startReading() async {
    ...
    _readTimer = Timer.periodic(Duration(milliseconds: 500), (_) async {
      ...
      try {
        ...
        final data = await _wristCharacteristic!.read().timeout(
          Duration(seconds: 2),
          onTimeout: () {
            print('Read timeout');
            return [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Wrist: ${widget.device.name}'),
        actions: [
          Padding(
